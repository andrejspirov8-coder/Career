...
test('dashboard pages require login and logout clears the protected session', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Unlock dashboard' }).click()
  await expect(page.getByRole('heading', { name: 'Recruiters' })).toBeVisible()
  const sessionCookie = (await page.context().cookies()).find((cookie) => cookie.name === 'career_dashboard_session')
  expect(sessionCookie?.httpOnly).toBe(true)
  expect(sessionCookie?.sameSite).toBe('Strict')
  expect(sessionCookie?.value).not.toContain(dashboardToken)
  expect(sessionCookie?.expires || 0).toBeGreaterThan(Date.now() / 1000 + 29 * 24 * 60 * 60)
