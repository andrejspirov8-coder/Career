...
from career_job_search.cvs.matching import PROFILES_PATH
from career_job_search.recruiters.history import company_history as _company_history
from career_job_search.recruiters.persona_stats import load_persona_stats

CV_DIR = project_path("cv")

_TOOL_REGISTRY: dict[str, Any] = {}


class _LocalTool:
    """Minimal LangChain-compatible tool fallback for offline tests/dev."""
...
def _register_local_tools() -> dict[str, Any]:
    _TOOL_REGISTRY.update(
        {
            "lookup_company_history": _LocalTool(
                "lookup_company_history", _tool_lookup_company_history
            ),
            "lookup_persona_stats": _LocalTool(
                "lookup_persona_stats", _tool_lookup_persona_stats
            ),
            "lookup_candidate_evidence": _LocalTool(
                "lookup_candidate_evidence", _tool_lookup_candidate_evidence
            ),
            "count_chars": _LocalTool("count_chars", _tool_count_chars),
            "get_cv_blurb": _LocalTool("get_cv_blurb", _tool_get_cv_blurb),
        }
    )
