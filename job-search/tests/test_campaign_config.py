...
def test_load_config_accepts_known_schema_version(tmp_path) -> None:
    ...
    config.write_text(f"schema_version: {CAMPAIGN_CONFIG_SCHEMA_VERSION!r}\nlimits:\n  a: 1\n")
...
def test_load_config_rejects_unknown_schema_version(tmp_path) -> None:
    ...
    with pytest.raises(SystemExit, match="Unsupported linkedin/config.yaml schema_version"):
        load_config(config)
