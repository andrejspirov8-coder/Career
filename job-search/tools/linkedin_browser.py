...
from linkedin_profile_lock import release_stale_chrome_profile_lock
from recruiter_linkedin_paths import PROFILE_DIR
...
def resolve_browse_binary() -> str:
    explicit = (
        os.environ.get("JOB_SEARCH_BROWSE_CLI")
        or os.environ.get("BROWSE_CLI")
        or ""
    ).strip()
    if explicit:
        return explicit
...
class BrowseWsLinkedInAutomator(LinkedInAutomatorBase):
    ...
    def _run_raw(self, pieces: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run(
            ...
            full, capture_output=True, text=True, timeout=180
    ...
    def goto(self, url: str) -> None:
        ...
        self._run(
            ...
            ["open", "--wait", "domcontentloaded", "--timeout", "45000", url]
...
def start_browse_ws_session(cfg: dict[str, Any]) -> tuple[BrowseWsLinkedInAutomator, BrowseChromeDaemon]:
