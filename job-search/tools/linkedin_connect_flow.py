...
def _click_connect_candidates_in(root: Any, *, timeout_ms: int = 7000) -> bool:
    ...
    try:
        ...
        text_btn = root.locator("button").filter(has_text=re.compile(r"^connect$", re.I)).first
...
def click_profile_connect(
    ...
    *,
    ...
    try:
        page.get_by_role("link", name=connect_re).first.wait_for(state="visible", timeout=12_000)
    except Exception:
        pass
    ...
    try:
        ...
        if not more_btn.is_visible(timeout=1200):
            ...
            more_btn = page.get_by_role("button", name=re.compile(r"^more\b", re.I)).first
...
def playwright_try_send_invitation(
    ...
    *,
    ...
    try:
        dialog = page.locator('[role="dialog"]').filter(has_text=re.compile(r"(invite|invitation)", re.I)).first
    ...
    try:
        ...
        if add_note_candidates.count() >= 1 and add_note_candidates.first.is_visible(timeout=2500):
