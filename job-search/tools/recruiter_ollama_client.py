...
def _record_ollama_failure(full_cfg: dict[str, Any], error: str) -> None:
    ...
    if _OLLAMA_FAIL_COUNT >= threshold:
        ...
        try:
            from recruiter_llm_trace import emit_event
...
def invoke_structured(
    ...
    model = resolve_chat_model(full_cfg, agent_name)
    from recruiter_llm_trace import AgentCallTrace
...
def invoke_with_tools(
    ...
    *,
    ...
    model = resolve_chat_model(full_cfg, agent_name)
    from recruiter_agent_tools import dispatch_tool_call
    from recruiter_llm_trace import AgentCallTrace, emit_event
